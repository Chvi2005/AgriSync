<?php
/**
 * AgriSync — Dedicated Accept Match API Endpoint (TASK-049 / TASK-073)
 * Handles farmer or admin match acceptance, status transitions, and notifications.
...
if (!in_array($user_role, ['farmer', 'admin'], true)) {
    jsonResponse(false, [], 'Access denied: Only farmers and administrators can accept match proposals.', 403);
}
...
try {
    ...
    $stmt_m = $db->prepare("UPDATE order_matches SET status = 'accepted', updated_at = NOW() WHERE id = ?");
    $stmt_m->execute([$match_id]);

    // Update listing status to 'matched'
    $stmt_l = $db->prepare("UPDATE harvest_listings SET status = 'matched', updated_at = NOW() WHERE id = ?");
    $stmt_l->execute([$match['listing_id']]);

    // Update order request status to 'matched'
    $stmt_o = $db->prepare("UPDATE order_requests SET status = 'matched', updated_at = NOW() WHERE id = ?");
    $stmt_o->execute([$match['order_id']]);

    // Send in-app notification to business buyer
    $notif_msg = "🌾 Farmer accepted your order match #M-{$match_id} for {$match['crop_type']} at Rs. " . number_format($match['matched_price'], 2) . "/kg.";
    $notif_stmt = $db->prepare("
        INSERT INTO notifications (user_id, message, link, is_read, created_at, updated_at)
        VALUES (?, ?, ?, 0, NOW(), NOW())
    ");
    $notif_stmt->execute([$match['business_id'], $notif_msg, '/business/orders.php']);
    ...
    jsonResponse(true, [
        ...
        'status'   => 'accepted'
    ], 'Match proposal accepted successfully!');

} catch (PDOException $e) {
    if ($db->inTransaction()) {
        $db->rollBack();
    }
    jsonResponse(false, [], 'Failed to accept order match.', 500);
}
